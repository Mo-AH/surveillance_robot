...
This module implements the node that manages the robot 2D pose and the battery level.
...
from surveillance_robot.srv import GetPose, GetPoseResponse, SetPose, SetPoseResponse
...
class RobotState:
    ...
    def __init__(self):
        ...
        if self._randomness:
            self._random_battery_time = rospy.get_param(anm.PARAM_BATTERY_TIME, [40.0, 80.0])
        ...
        log_msg = (f'Initialise node `{anm.NODE_ROBOT_STATE}` with services `{anm.SERVER_GET_POSE}` and '
                   f'`{anm.SERVER_SET_POSE}`, {anm.SERVER_CHARGE} and topic {anm.TOPIC_BATTERY_LOW}.')
        rospy.loginfo(anm.tag_log(log_msg, LOG_TAG))
    ...
    def set_pose(self, request):
        """ The `robot/set_pose` service implementation.
            ...
            Args:
                request (SetPoseRequest) : request with the current robot pose to be set,

            Returns:
                response (SetPoseResponse) : empty response
    ...
    def get_pose(self, request):
        """ The `robot/get_pose` service implementation.
            ...
            Args:
                request (GetPoseRequest) : empty request

            Returns:
                response (SetPoseResponse) : response with the current pose of the robot
